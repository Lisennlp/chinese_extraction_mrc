...
from model.modeling import BertForQuestionAnswering, BertConfig
from model.optimization import BertAdam, WarmupLinearSchedule
from model.tokenization import (BasicTokenizer, BertTokenizer)
...
def read_squad_examples(input_file, is_training, version_2_with_negative):
    ...
    with open(input_file, "r") as reader:
        for lidx, line in enumerate(reader):
            ...
            if is_training and len(sample['answer_docs']) == 0:    # 过滤掉不是答案的
                continue
            ...
            if len(sample['answer_spans']) == 0:    # 如果没有答案，过滤掉
                continue
            if sample['answer_spans'][0][1] >= 500:    # 如果答案长度>500，过滤掉
                continue
            # 答案在的起始位置，这里的位置指的是分词后在most_related_para_seg列表中的索引，因为most_related_para_seg虽然是答案分词列表，但是不一定都是答案，所以有起始，结束索引
            start_position = sample['answer_spans'][0][0]
            end_position = sample['answer_spans'][0][1]    # 答案的结束位置
            orig_answer_text = sample['fake_answers'][0]    # 初始答案，也可以说是标注答案
            example = SquadExample(
                ...
                is_impossible=is_impossible    # 选的都是对的答案，所以都为False
            )
            examples.append(example)

    data_type = 'train' if is_training else 'eval'
    print(f"最终的{data_type}数据长度为：", len(examples))
    return examples    # 存了example对象的列表，之后就可以铜鼓example.属性的方式获取其属性值
...
def write_predictions(all_examples, all_features, all_results, n_best_size, max_answer_length,
                      ...
                      output_null_log_odds_file, verbose_logging, version_2_with_negative,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            print(f'result start logit = {result.start_logits}')
        ...
        # 按照起始位置分数+结束位置分数之和从大到小排序
        prelim_predictions = sorted(prelim_predictions,
                                    key=lambda x: (x.start_logit + x.end_logit),
                                    reverse=True)

        _NbestPrediction = collections.namedtuple(    # pylint: disable=invalid-name
            "NbestPrediction", ["text", "start_logit", "end_logit", "question"])
        ...
        for (i, entry) in enumerate(nbest):
            ...
            output["question"] = entry.question
        ...
        if not version_2_with_negative:
            all_predictions[example.qas_id] = [nbest_json[0]["question"], nbest_json[0]["text"]]
        else:
...
def prepare_data(examples, tokenizer, args, task_name='train'):
    is_training = 0 if task_name == 'predict' else 1
    train_features = convert_examples_to_features(examples=examples,
                                                  tokenizer=tokenizer,
    ...
    if task_name == 'predict':
        return train_dataloader, train_features
    else:
...
def main():
    ...
    if args.do_train:
        ...
        train_dataloader = prepare_data(train_examples, tokenizer, args, task_name="train")
    ...
    def eval_model(model, data_loader):
        ...
        for step, batch in enumerate(
                tqdm(data_loader, desc="Evaluating", disable=args.local_rank not in [-1, 0])):
            if n_gpu == 1:
    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            for step, batch in enumerate(
                    tqdm(train_dataloader, desc="Training", disable=args.local_rank not in [-1,
                                                                                            0])):
                if n_gpu == 1:
                    batch = tuple(
            ...
            print(f'epoch {epoch} train loss {train_loss / (step + 1)} eval_loss {eval_loss}')
        ...
        for input_ids, input_mask, segment_ids, example_indices in tqdm(
                predict_dataloader, desc="Predicting", disable=args.local_rank not in [-1, 0]):
        ...
        write_predictions(eval_examples, predict_features, all_results, args.n_best_size,
